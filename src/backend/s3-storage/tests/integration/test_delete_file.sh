# Integration test: POST /files/delete — soft delete file
# Given: bucket with uploaded file
# When: POST /files/delete with fileId
...
TMP_FILE=$(mktemp)
echo "test content for delete" > "$TMP_FILE"
...
upload_resp=$(curl -sS -w "\n%{http_code}" -X POST \
    -H "X-User-Id: $USER_ID" \
    -F "file=@${TMP_FILE};filename=test_delete.txt" \
...
if [[ "$delete_status" != "200" ]]; then
    echo "FAIL: Delete file expected 200, got $delete_status"
    echo "$delete_body"
    exit 1
fi
...
echo "PASS: Delete file returns 200, file not in list"
exit 0
