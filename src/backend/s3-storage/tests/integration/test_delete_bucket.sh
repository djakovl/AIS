# Integration test: POST /files/buckets/delete — soft delete empty bucket
# Given: empty bucket
# When: POST /files/buckets/delete with bucketId
...
if [[ "$delete_status" != "200" ]]; then
    echo "FAIL: Delete bucket expected 200, got $delete_status"
    echo "$delete_body"
    exit 1
fi
...
echo "PASS: Delete empty bucket returns 200, bucket not in list"
exit 0
